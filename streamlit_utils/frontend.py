...
# Old version of filters (multiple selection, deletion mechanism, etc.). Leave it here until business requests
...
#     st.divider()
#
#     df_with_remove = st.session_state.selected_filters.copy()
#     df_with_remove["Delete"] = False
#
#     cols = ["Delete"] + [col for col in df_with_remove.columns if col != "Delete"]
#     df_with_remove = df_with_remove[cols]
#
...
#             st.session_state.selected_filters = add_filter(dynamic_filters)
#             st.rerun()
#
#     with col2:
#         if st.button("Apply", use_container_width=True):
#             st.session_state.base_matrix_scope = st.session_state.selected_filters
#
#     with col3:
#         if st.button("Drop selected", use_container_width=True):
#             updated_df = edited_df[~edited_df["Delete"]] \
#                 .drop(columns="Delete") \
#                 .reset_index(drop=True)
#
#             updated_df = updated_df.dropna(axis=1, how='all')
#             st.session_state.selected_filters = updated_df
#             st.rerun()
#
#
# def add_filter(dynamic_filters):
